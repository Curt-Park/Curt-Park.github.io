...
Network의 loss function $$\big( r + \gamma max_{a'}Q(s', a'; \theta_i^{-} - Q(s, a; \theta_i) \big)^2$$에 대한 gradient가 -1 보다 작을때는 -1로, 1보다 클때는 1로 clipping해준다[[2](https://github.com/kuz/DeepMind-Atari-Deep-Q-Learner/blob/master/dqn/NeuralQLearner.lua#L224)]. 이러한 방식은 loss function이 일정 구간 내에서는 L2로, 구간 바깥에서는 L1으로 동작하게 하는데, 이는 outlier에 강한 성질이 있으므로 학습과정을 좀 더 안정적으로 만들 수 있다. Huber loss[7]와 기능적으로 동일하기 때문에 loss function을 Huber loss로 정의하기도 하는 것으로 보인다[[8](https://github.com/devsisters/DQN-tensorflow/issues/16)].
